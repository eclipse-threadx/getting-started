...
{
    if (iot_mqtt == NULL || iot_mqtt->cb_ptr_mqtt_invoke_direct_method != NULL)
    {
        return NX_PTR_ERROR;
    }
    
    iot_mqtt->cb_ptr_mqtt_invoke_direct_method = mqtt_direct_method_callback;
    return NX_SUCCESS;
}

UINT iot_mqtt_register_c2d_message_callback(IOT_MQTT *iot_mqtt, func_ptr_c2d_message mqtt_c2d_message_callback)
{
    if (iot_mqtt == NULL || iot_mqtt->cb_ptr_mqtt_c2d_message != NULL)
    {
        return NX_PTR_ERROR;
    }
    
    iot_mqtt->cb_ptr_mqtt_c2d_message = mqtt_c2d_message_callback;
    return NX_SUCCESS;
}
...
UINT iot_mqtt_register_device_twin_desired_prop_callback(IOT_MQTT *iot_mqtt, func_ptr_device_twin_desired_prop mqtt_device_twin_desired_prop_callback)
{
    ...
    iot_mqtt->cb_ptr_mqtt_device_twin_desired_prop_callback = mqtt_device_twin_desired_prop_callback;
    return NX_SUCCESS;
}
...
static UINT tls_setup(NXD_MQTT_CLIENT *client, NX_SECURE_TLS_SESSION *tls_session, NX_SECURE_X509_CERT *cert, NX_SECURE_X509_CERT *trusted_cert)
{
    ...
    IOT_MQTT *iot_mqtt = (IOT_MQTT *)client->nxd_mqtt_packet_receive_context;
    
    for (UINT index = 0; index < TLS_REMOTE_CERTIFICATE_COUNT; ++index)
    {
        status = nx_secure_tls_remote_certificate_allocate(
            tls_session,
            &iot_mqtt->tls_remote_certificate[index],
            iot_mqtt->tls_remote_cert_buffer[index],
            sizeof(iot_mqtt->tls_remote_cert_buffer[index]));
    ...
    status = nx_secure_tls_session_packet_buffer_set(
        tls_session,
        iot_mqtt->tls_packet_buffer,
        sizeof(iot_mqtt->tls_packet_buffer));
    if (status != NX_SUCCESS)
    {
    ...
    nx_secure_tls_session_time_function_set(tls_session, iot_mqtt->unix_time_get);

    return NX_SUCCESS;
}

static UINT mqtt_publish(IOT_MQTT *iot_mqtt, CHAR *topic, CHAR *message)
{
    UINT status = nxd_mqtt_client_publish(
        &iot_mqtt->nxd_mqtt_client,
        topic, strlen(topic),
...
static UINT mqtt_publish_float(IOT_MQTT *iot_mqtt, CHAR *topic, CHAR *label, float value)
{
    ...
    return mqtt_publish(iot_mqtt, topic, mqtt_message);
}

static UINT mqtt_publish_bool(IOT_MQTT *iot_mqtt, CHAR  *topic, CHAR *label, bool value)
{
    CHAR mqtt_message[200] = { 0 };
...
static UINT mqtt_publish_string(IOT_MQTT *iot_mqtt, CHAR  *topic, CHAR *label, CHAR *value)
{
    CHAR mqtt_message[200] = { 0 };
...
static UINT mqtt_respond_direct_method(IOT_MQTT *iot_mqtt, CHAR *topic, CHAR *request_id, MQTT_DIRECT_METHOD_RESPONSE *response)
{
    snprintf(topic, MQTT_TOPIC_NAME_LENGTH, DIRECT_METHOD_RESPONSE, response->status, request_id);
    return mqtt_publish(iot_mqtt, topic, response->message);
}

static VOID process_direct_method(IOT_MQTT *iot_mqtt, CHAR *topic, CHAR *message)
{
    ...
    if (iot_mqtt->cb_ptr_mqtt_invoke_direct_method == NULL)
    {
        printf("No callback is registered for MQTT direct method invoke\r\n");
        return;
    }

    iot_mqtt->cb_ptr_mqtt_invoke_direct_method(direct_method_name, message, &response);

    mqtt_respond_direct_method(iot_mqtt, topic, request_id, &response);
}

static VOID process_device_twin_response(IOT_MQTT *iot_mqtt, CHAR *topic)
{
...
static VOID process_c2d_message(IOT_MQTT *iot_mqtt, CHAR *topic)
{
    CHAR key[64] = { 0 };
    CHAR value[64] = { 0 };
    ...
    if (iot_mqtt->cb_ptr_mqtt_c2d_message == NULL)
    {
        printf("No callback is registered for MQTT cloud to device message processing\r\n");
        return;
    }
    
    iot_mqtt->cb_ptr_mqtt_c2d_message(key, value);
}

static VOID process_device_twin_desired_prop_update(IOT_MQTT *iot_mqtt, CHAR *topic, CHAR *message)
{
    iot_mqtt->cb_ptr_mqtt_device_twin_desired_prop_callback(message);
}
...
static VOID mqtt_notify_cb(NXD_MQTT_CLIENT *client_ptr, UINT number_of_messages)
{
    ...
    IOT_MQTT *iot_mqtt = (IOT_MQTT *)client_ptr->nxd_mqtt_packet_receive_context;

    tx_mutex_get(&iot_mqtt->iot_mqtt_mutex, TX_WAIT_FOREVER);
    ...
    if (status == NXD_MQTT_SUCCESS)
    {
        ...
        {
            process_direct_method(iot_mqtt, mqtt_topic_buffer, mqtt_message_buffer);
        }
        else if (strstr((CHAR *)mqtt_topic_buffer, DEVICE_TWIN_RES_BASE))
        {
            process_device_twin_response(iot_mqtt, mqtt_topic_buffer);
        }
        else if (strstr((CHAR *)mqtt_topic_buffer, DEVICE_MESSAGE_BASE))
        {
            process_c2d_message(iot_mqtt, mqtt_topic_buffer);
        }
        else if (strstr((CHAR *)mqtt_topic_buffer, DEVICE_TWIN_DESIRED_PROP_RES_BASE))
        {
            process_device_twin_desired_prop_update(iot_mqtt, mqtt_topic_buffer, mqtt_message_buffer);
        }
        else
        {
    ...
    tx_mutex_put(&iot_mqtt->iot_mqtt_mutex);
}
...
UINT iot_mqtt_publish_float_telemetry(IOT_MQTT *iot_mqtt, CHAR* label, float value)
{
    ...
    snprintf(mqtt_publish_topic, sizeof(mqtt_publish_topic), PUBLISH_TELEMETRY_TOPIC, iot_mqtt->nxd_mqtt_client.nxd_mqtt_client_id);
    printf("Sending telemetry\r\n");

    status =  mqtt_publish_float(iot_mqtt, mqtt_publish_topic, label, value);
...
UINT iot_mqtt_publish_bool_property(IOT_MQTT *iot_mqtt, CHAR* label, bool value)
{
    ...
    status = mqtt_publish_bool(iot_mqtt, mqtt_publish_topic, label, value);
...
UINT iot_mqtt_publish_string_property(IOT_MQTT *iot_mqtt, CHAR* label, CHAR *value)
{
    ...
    status = mqtt_publish_string(iot_mqtt, mqtt_publish_topic, label, value);
    ...
    CHAR *iot_model_id)
{
    ...
    iot_mqtt->unix_time_get = unix_time_get;
    iot_mqtt->iot_mqtt_device_id = iot_device_id;
    iot_mqtt->iot_mqtt_sas_key = iot_sas_key;
    iot_mqtt->iot_mqtt_hub_hostname = iot_hub_hostname;
    iot_mqtt->iot_mqtt_model_id = iot_model_id;

    status = nxd_mqtt_client_create(
        &iot_mqtt->nxd_mqtt_client, "MQTT client",
        iot_mqtt->iot_mqtt_device_id, strlen(iot_mqtt->iot_mqtt_device_id),
        nx_ip, nx_pool,
        iot_mqtt->iot_mqtt_client_stack, IOT_MQTT_CLIENT_STACK_SIZE, 
    ...
    if (status)
    {
        ...
        nxd_mqtt_client_delete(&iot_mqtt->nxd_mqtt_client);
        return status;
    }
    ...
    status = tx_mutex_create(&iot_mqtt->iot_mqtt_mutex, "Azure MQTT", TX_NO_INHERIT);
    ...
    iot_mqtt->nxd_mqtt_client.nxd_mqtt_packet_receive_context = iot_mqtt;
...
{
    nxd_mqtt_client_disconnect(&iot_mqtt->nxd_mqtt_client);
    nxd_mqtt_client_delete(&iot_mqtt->nxd_mqtt_client);
    tx_mutex_delete(&iot_mqtt->iot_mqtt_mutex);
...
UINT iot_mqtt_connect(IOT_MQTT *iot_mqtt)
{
    ...
    printf("\tHub hostname: %s\r\n", iot_mqtt->iot_mqtt_hub_hostname);
    printf("\tDevice id: %s\r\n", iot_mqtt->iot_mqtt_device_id);
    printf("\tModel id: %s\r\n", iot_mqtt->iot_mqtt_model_id);

    // Create the username & password
    snprintf(iot_mqtt->iot_mqtt_username, IOT_MQTT_USERNAME_SIZE, USERNAME, iot_mqtt->iot_mqtt_hub_hostname, iot_mqtt->iot_mqtt_device_id, iot_mqtt->iot_mqtt_model_id);
    
    if (!create_sas_token(
        iot_mqtt->iot_mqtt_sas_key, strlen(iot_mqtt->iot_mqtt_sas_key),
        iot_mqtt->iot_mqtt_hub_hostname, iot_mqtt->iot_mqtt_device_id, 
        iot_mqtt->unix_time_get(),
        iot_mqtt->iot_mqtt_password, IOT_MQTT_PASSWORD_SIZE))
    {
    ...
    status = nx_secure_tls_session_create(
        &iot_mqtt->nxd_mqtt_client.nxd_mqtt_tls_session,
        &nx_crypto_tls_ciphers,
        iot_mqtt->tls_metadata_buffer,
        sizeof(iot_mqtt->tls_metadata_buffer));
    if (status != NXD_MQTT_SUCCESS)
    {
    ...
    status = nxd_mqtt_client_login_set(
        &iot_mqtt->nxd_mqtt_client,
        iot_mqtt->iot_mqtt_username, strlen(iot_mqtt->iot_mqtt_username),
        iot_mqtt->iot_mqtt_password, strlen(iot_mqtt->iot_mqtt_password));
    if (status != NXD_MQTT_SUCCESS)
    {
        printf("Could not create Login Set (0x%02x)\r\n", status);
        nx_secure_tls_session_delete(&iot_mqtt->nxd_mqtt_client.nxd_mqtt_tls_session);
        return status;
    }

    // Resolve the MQTT server IP address
    status = nxd_dns_host_by_name_get(iot_mqtt->nx_dns, (UCHAR *)iot_mqtt->iot_mqtt_hub_hostname, &server_ip, NX_IP_PERIODIC_RATE, NX_IP_VERSION_V4);
    if (status != NX_SUCCESS)
    {
        printf("Unable to resolve DNS for MQTT Server %s (0x%02x)\r\n", iot_mqtt->iot_mqtt_hub_hostname, status);
        nx_secure_tls_session_delete(&iot_mqtt->nxd_mqtt_client.nxd_mqtt_tls_session);
        return status;
    }
    ...
    snprintf(mqtt_subscribe_topic, sizeof(mqtt_subscribe_topic), DEVICE_MESSAGE_TOPIC, iot_mqtt->iot_mqtt_device_id);
...
{
    UINT status = nxd_mqtt_client_disconnect(&iot_mqtt->nxd_mqtt_client);
