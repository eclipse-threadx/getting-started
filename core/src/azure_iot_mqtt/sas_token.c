    ...
    unsigned int output_size)
{
    ...
    output += snprintf(output, output_end - output, "SharedAccessSignature sr=%s/devices/%s&sig=", hostname, device_id);
    ...
    unsigned int output_size)
{
    ...
    valid_until += SAS_EXPIRATION_SECS;
    snprintf(buffer, sizeof(buffer), "%s/registrations/%s\n%lu", id_scope, registration_id, valid_until);
    ...
    // Create the output SAS token
    output += snprintf(output, output_end - output, "SharedAccessSignature sr=%s/registrations/%s&sig=", id_scope, registration_id);
    output += url_encode(output, encoded_hash);
