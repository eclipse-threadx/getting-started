...
/*    message (to be sent by the caller).                                 */
/*                                                                        */
/*  INPUT                                                                 */
/*                                                                        */
...
/*                                            fix swapped PSK parameter,  */
/*                                            improved packet length      */
/*                                            verification, updated       */
/*                                            error return checks, keep   */
/*                                            certificates if no errors,  */
...
NX_CRYPTO_EXTENDED_OUTPUT             extended_output;
    ...
    NX_SECURE_SEND_CLIENT_KEY_EXCHANGE_EXTENSION
    ...
    {
        if ((tls_session -> nx_secure_tls_credentials.nx_secure_tls_client_psk.nx_secure_tls_psk_id_size >
             sizeof(tls_session -> nx_secure_tls_credentials.nx_secure_tls_client_psk.nx_secure_tls_psk_id)) ||
...
#endif
    {
        ...
        if ((data_size < NX_SECURE_TLS_PREMASTER_SIZE) ||
        ...
        for (i = 2; i < (data_size - NX_SECURE_TLS_PREMASTER_SIZE - 1); ++i)
        {
        ...
        NX_SECURE_MEMCPY(&_nx_secure_client_padded_pre_master[data_size - NX_SECURE_TLS_PREMASTER_SIZE],
               tls_session -> nx_secure_tls_key_material.nx_secure_tls_pre_master_secret, NX_SECURE_TLS_PREMASTER_SIZE);
