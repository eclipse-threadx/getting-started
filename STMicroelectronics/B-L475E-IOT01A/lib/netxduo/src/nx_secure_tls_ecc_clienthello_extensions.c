...
static UINT _nx_secure_tls_ecc_send_clienthello_sig_extension(NX_SECURE_TLS_SESSION *tls_session,
                                                              UCHAR *packet_buffer, ULONG *packet_offset,
                                                              USHORT *extension_length,
                                                              ULONG available_size);
static UINT _nx_secure_tls_ecc_send_clienthello_sni_extension(NX_SECURE_TLS_SESSION *tls_session,
                                                              UCHAR *packet_buffer, ULONG *packet_offset,
                                                              USHORT *extension_length,
                                                              ULONG available_size);
static UINT _nx_secure_tls_ecc_send_clienthello_sec_spf_extensions(NX_SECURE_TLS_SESSION *tls_session,
                                                                   UCHAR *packet_buffer, ULONG *packet_offset,
                                                                   USHORT *extension_length,
                                                                   ULONG available_size);

#ifdef NX_SECURE_TLS_ENABLE_SECURE_RENEGOTIATION
static UINT _nx_secure_tls_ecc_send_clienthello_sec_reneg_extension(NX_SECURE_TLS_SESSION *tls_session,
                                                                    UCHAR *packet_buffer,
                                                                    ULONG *packet_offset,
                                                                    USHORT *extension_length,
                                                                    ULONG available_size);
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_secure_tls_ecc_send_clienthello                 PORTABLE C      */
/*                                                           5.12         */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*  CALLS                                                                 */
/*                                                                        */
/*    _nx_secure_tls_ecc_send_clienthello_sec_spf_extensions              */
/*                                          Send ClientHello Supported    */
/*                                            Elliptic Curves and Point   */
/*                                            Format extension            */
/*    _nx_secure_tls_ecc_send_clienthello_sec_reneg_extension             */
/*                                          Send ClientHello Renegotiation*/
/*                                            extension                   */
/*    _nx_secure_tls_ecc_send_clienthello_sig_extension                   */
/*                                          Send ClientHello Signature    */
/*                                            extension                   */
/*    _nx_secure_tls_ecc_send_clienthello_sni_extension                   */
/*                                          Send ClientHello SNI extension*/
/*    _nx_secure_tls_ecc_send_clienthello_ec_extension                    */
...
USHORT extension_length = 0, total_extensions_length;
    ...
    _nx_secure_tls_ecc_send_clienthello_sec_spf_extensions(tls_session,
                                                           packet_buffer,
                                                           &length,
                                                           &extension_length,
                                                           available_size);
    ...
    {
        _nx_secure_tls_ecc_send_clienthello_sec_reneg_extension(tls_session, packet_buffer, &length, &extension_length, available_size);
        total_extensions_length = (USHORT)(total_extensions_length + extension_length);
    }
#endif

    /* Send the available signature algorithms extension. */
    _nx_secure_tls_ecc_send_clienthello_sig_extension(tls_session, packet_buffer, &length, &extension_length, available_size);
    total_extensions_length = (USHORT)(total_extensions_length + extension_length);

#ifndef NX_SECURE_TLS_SNI_EXTENSION_DISABLED
    /* Send the server name indication extension. */
    _nx_secure_tls_ecc_send_clienthello_sni_extension(tls_session, packet_buffer, &length, &extension_length, available_size);
    total_extensions_length = (USHORT)(total_extensions_length + extension_length);
#endif
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_secure_tls_ecc_send_clienthello_sig_extension   PORTABLE C      */
/*                                                           5.12         */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Timothy Stapko, Express Logic, Inc.                                 */
...
/*  CALLED BY                                                             */
/*                                                                        */
/*    _nx_secure_tls_ecc_send_clienthello_extensions                      */
...
#define TLS_SIG_EXTENSION_SHA256_ECDSA 0x0403
    ...
    if (available_size < (*packet_offset + total_length))
    {
    ...
    if (ecc_info -> nx_secure_tls_ecc_supported_groups_count != 0)
    {
        sighash_len = (USHORT)(sighash_len + 4);
    }
    ...
    if (ecc_info -> nx_secure_tls_ecc_supported_groups_count != 0)
    {
        algo[0] = TLS_SIG_EXTENSION_SHA1_ECDSA;
        algo[1] = TLS_SIG_EXTENSION_SHA256_ECDSA;
...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_secure_tls_ecc_send_clienthello_sni_extension   PORTABLE C      */
/*                                                           5.12         */
/*  AUTHOR                                                                */
/*                                                                        */
/*    Timothy Stapko, Express Logic, Inc.                                 */
...
NX_SECURE_TLS_ECC *ecc_info;
    ...
    if (available_size < (*packet_offset + 12u + (ULONG)(ecc_info -> nx_secure_tls_ecc_supported_groups_count << 1)))
    {
