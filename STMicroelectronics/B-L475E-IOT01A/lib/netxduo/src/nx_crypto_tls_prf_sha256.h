...
#define NX_SECURE_TLS_PRF_SHA256                NX_CRYPTO_TLS_PRF_SHA256

typedef struct NX_CRYPTO_TLS_PRF_SHA256_STRUCT
{
    ...
    UCHAR nx_secure_tls_prf_temp_hmac_output_buffer[32]; /* The temp buffer for the output buffer of hmac(secret, A(i) + seed) */
    UCHAR nx_secure_tls_prf_hmac_metadata_area[sizeof(NX_CRYPTO_SHA256_HMAC)]; /* metadata buffer for the hmac function */
} NX_CRYPTO_TLS_PRF_SHA256;

UINT _nx_crypto_method_prf_sha_256_init(struct NX_CRYPTO_METHOD_STRUCT *method,
                                        UCHAR *key, NX_CRYPTO_KEY_SIZE key_size_in_bits,
                                        VOID **handle,
                                        VOID *crypto_metadata,
                                        ULONG crypto_metadata_size);

UINT _nx_crypto_method_prf_sha_256_cleanup(VOID *crypto_metadata);

UINT _nx_crypto_method_prf_sha_256_operation(UINT op,      /* Encrypt, Decrypt, Authenticate */
