...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_secure_x509_ecc_certificate_parse               PORTABLE C      */
/*                                                          5.12          */
/*  AUTHOR                                                                */
/*                                                                        */
...
UINT _nx_secure_x509_ecc_certificate_parse(const UCHAR *buffer, UINT length, UINT *bytes_processed,
                                           struct NX_SECURE_X509_CERT_STRUCT *cert)
{
USHORT       tlv_type;
    ...
    *bytes_processed += header_length;
    while (length > 0)
    {
        ...
        *bytes_processed += (header_length + tlv_length);
        ...
        {
            _nx_secure_x509_ecc_oid_parse(tlv_data, tlv_length, &oid);
...
/*    _nx_secure_x509_ecc_oid_parse         Parse OID in certificate      */
/*                                                                        */
/*  CALLED BY                                                             */
/*                                                                        */
/*    _nx_secure_x509_parse_cert_data       Parse certificate             */
...
UINT         status;
    ...
    *bytes_processed += (header_length + tlv_length);
    ...
    if (tlv_type == NX_SECURE_ASN_TAG_OID)
    {
        ...
        {
            /* The OID is in the data we extracted. */
            _nx_secure_x509_ecc_oid_parse(tlv_data, tlv_length, &oid_parameter);
