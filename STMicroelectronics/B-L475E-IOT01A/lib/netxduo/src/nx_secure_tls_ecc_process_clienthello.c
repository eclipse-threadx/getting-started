...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _nx_secure_tls_ecc_process_clienthello              PORTABLE C      */
/*                                                          5.12          */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*    _nx_secure_tls_ecc_process_clienthello_extensions                   */
...
NX_SECURE_TLS_ECDHE_HANDSHAKE_DATA   *ecdhe_data;
    ...
    {
        ec_pubkey = (NX_SECURE_EC_PUBLIC_KEY *)&cert -> nx_secure_x509_public_key;
    ...
    status = _nx_secure_tls_ecc_process_clienthello_extensions(tls_session,
                                                               extension_data,
                                                               num_extensions,
                                                               &selected_curve,
    ...
    for (i = 0; i < ciphersuite_list_length; i += 2)
    {
        ...
        if((cipher_entry == TLS_FALLBACK_NOTIFY_SCSV) && (tls_session -> nx_secure_tls_protocol_version_override == 0))
        {
                                             ...
                                             UINT cert_curve_supported)
{
    ...
    case NX_CRYPTO_KEY_EXCHANGE_ECDHE:
        ...
        if (ciphersuite_info -> nx_secure_tls_public_auth -> nx_crypto_algorithm == NX_CRYPTO_DIGITAL_SIGNATRUE_ECDSA)
        {
    ...
    case NX_CRYPTO_KEY_EXCHANGE_ECDH:
        /* Check for ECDH_anon. */
        if (ciphersuite_info -> nx_secure_tls_public_auth -> nx_crypto_algorithm == NX_CRYPTO_DIGITAL_SIGNATRUE_ANONYMOUS)
        {
