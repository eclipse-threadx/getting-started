...
NX_CRYPTO_PHASH *phash;
    ...
    if(crypto_metadata_size < sizeof(NX_CRYPTO_TLS_PRF_1))
    {
        return(NX_CRYPTO_PTR_ERROR);
    }

    /* Get our control block. */
    prf = (NX_CRYPTO_TLS_PRF_1 *)crypto_metadata;
...
NX_CRYPTO_KEEP UINT  _nx_crypto_method_prf_1_cleanup(VOID *crypto_metadata)
{
    ...
    NX_CRYPTO_MEMSET(crypto_metadata, 0, sizeof(NX_CRYPTO_TLS_PRF_1));
#else
...
/*  CALLS                                                                 */
/*                                                                        */
/*    _nx_crypto_tls_prf_1                  Implement the TLS PRF         */
/*                                            using SHA-1                 */
/*                                                                        */
/*  CALLED BY                                                             */
/*                                                                        */
...
NX_CRYPTO_PHASH *phash;
    ...
    phash -> nx_crypto_hmac_method = &crypto_method_hmac_md5;
    ...
    if (phash -> nx_crypto_phash_secret_length % 2)
    {
        ...
        secret_half_point = &(phash -> nx_crypto_phash_secret[secret_len - 1]);
    }
    else
    {
        /* Evenly divisible, offset is just after the half-length. */
        secret_half_point = &(phash -> nx_crypto_phash_secret[secret_len]);
    }
    ...
    /* Take use of the second half of the secret. */
    phash -> nx_crypto_phash_secret = secret_half_point;

    /* Do not clear the output buffer this time. */
    status = _nx_crypto_phash(phash, output, output_length_in_byte);
