                                           ...
                                           uint8_t tx_em_req)
{
  ...
  context->block = true;
  OSMutexCreate(&context->read_lock, "UART Read Lock", &err);
  EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  OSMutexCreate(&context->write_lock, "UART Write Lock", &err);
  EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  OSSemCreate(&context->read_signal, "UART Rx Wait", 0, &err);
  EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
#endif
                            ...
                            char c)
{
...
#if defined(SL_CATALOG_KERNEL_PRESENT)
  {
    ...
    OSSemPost(&uart_context->read_signal, OS_OPT_POST_ALL, &err);
    EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  }
...
static sl_status_t uart_deinit(void *stream)
{
  ...
  if (OSRunning == OS_STATE_OS_RUNNING) {
    ...
    OSMutexPend(&uart_context->write_lock, 0, OS_OPT_PEND_BLOCKING, NULL, &err);
    EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);  // If deinit is called twice in a row, the assert will trigger

    // Bypass lock if we print before the kernel is running
    OSMutexPend(&uart_context->read_lock, 0, OS_OPT_PEND_BLOCKING, NULL, &err);
    EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  }
#endif
  ...
  OSSemDel(&uart_context->read_signal, OS_OPT_DEL_ALWAYS, &err);
  EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  OSMutexDel(&uart_context->read_lock, OS_OPT_DEL_ALWAYS, &err);
  EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  OSMutexDel(&uart_context->write_lock, OS_OPT_DEL_ALWAYS, &err);
  EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
#endif
                              ...
                              size_t buffer_length)
{
  ...
  if (OSRunning == OS_STATE_OS_RUNNING) {
    // Bypass lock if we print before the kernel is running
    OSMutexPost(&uart_context->write_lock, OS_OPT_POST_NONE, &err);
    EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  }
#endif
  return SL_STATUS_OK;
                             ...
                             size_t *bytes_read)
{
  ...
  if (OSRunning == OS_STATE_OS_RUNNING) {
    ...
    if (uart_context->block) {
      OSSemPend(&uart_context->read_signal, 0, OS_OPT_PEND_BLOCKING, NULL, &err);
      EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
    }
  }
#endif
  ...
  *bytes_read = 0;
  ...
  if (OSRunning == OS_STATE_OS_RUNNING) {
    // Bypass lock if we print before the kernel is running
    OSMutexPost(&uart_context->read_lock, OS_OPT_POST_NONE, &err);
    EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
  }
#endif
...
static void set_rx_sem_count(sl_iostream_uart_context_t *uart_context)
{
  ...
  CORE_ENTER_ATOMIC();
  OSSemSet(&uart_context->read_signal, uart_context->rx_count, &err);
  CORE_EXIT_ATOMIC();
  EFM_ASSERT(RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE);
}
#endif
