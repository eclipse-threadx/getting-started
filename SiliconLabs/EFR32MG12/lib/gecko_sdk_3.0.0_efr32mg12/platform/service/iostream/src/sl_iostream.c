...
#if defined(SL_CATALOG_KERNEL_PRESENT)
static OS_REG_ID sli_task_register_id = TASK_ID_INVALID;
...
#if defined(SL_CATALOG_KERNEL_PRESENT)
  OSTaskRegSet(NULL, reg_id, (OS_REG)stream, &err);
...
sl_iostream_t *sl_iostream_get_default(void)
{
  ...
  if (reg_id != TASK_ID_INVALID) {
    ...
    reg = OSTaskRegGet(NULL, sli_task_register_id, &err);
