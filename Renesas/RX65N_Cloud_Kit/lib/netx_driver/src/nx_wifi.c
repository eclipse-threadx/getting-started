...
/////
#define WIFI_PAYLOAD_SIZE (1536u)
...
#define WIFI_TRANSPORT_TIMEOUT                   30000
...
static CHAR                         nx_wifi_buffer[WIFI_PAYLOAD_SIZE];
...
#ifdef NX_ENABLE_IP_PACKET_FILTER
    if (R_WIFI_SX_ULPGN_GetIpAddress(&ip_cfg) == WIFI_STATUS_OK)
    {
        nx_wifi_ip_address = ip_cfg.ipaddress;
    }
#endif /* NX_ENABLE_IP_PACKET_FILTER  */
        ...
        if (nx_wifi_socket_counter != 0)
        {
            ...
            for (i = 0; (i < NX_WIFI_SOCKET_COUNTER) && (socket_counter < nx_wifi_socket_counter); i++)
            {
                ...
                do
                {
                    ...
                    /* Make sure there is enough space to store the data before receiving data from WIFI.  */
                    if ((nx_wifi_pool -> nx_packet_pool_available * nx_wifi_pool -> nx_packet_pool_payload_size) <
                        (WIFI_PAYLOAD_SIZE + (NX_WIFI_PACKET_RESERVED * nx_wifi_pool -> nx_packet_pool_payload_size)))
                        break;

                    /* Receive the data in WIFI_READ_TIMEOUT ms.  */
                    size = R_WIFI_SX_ULPGN_ReceiveSocket(nx_wifi_socket[i].socket_id,
                            (uint8_t *)nx_wifi_buffer, WIFI_PAYLOAD_SIZE, WIFI_READ_TIMEOUT);
                    /* Check status.  */
                    if ((size <= 0))
                        break;
...
NX_UDP_SOCKET *udp_socket;
    ...
    else
    {
        ...
        while(total_millisecond)
        {
            ...
            /* Check if exceed the max value of WIFI_TIMEOUT.  */
            if (total_millisecond > WIFI_TIMEOUT)
                wait_millisecond = WIFI_TIMEOUT;
            else
                wait_millisecond = total_millisecond;

            /* Check if exceed the max value of WIFI_TRANSPORT_TIMEOUT.  */
            if (wait_millisecond > WIFI_TRANSPORT_TIMEOUT)
                wait_millisecond = WIFI_TRANSPORT_TIMEOUT;

            /* Receive the data within a specified time.  */
            size = R_WIFI_SX_ULPGN_ReceiveSocket(nx_wifi_socket[entry_index].socket_id,
                    (uint8_t *)nx_wifi_buffer, WIFI_PAYLOAD_SIZE, WIFI_READ_TIMEOUT);
...
UCHAR   entry_index;
    ...
    /* Create the socket. */
    status = R_WIFI_SX_ULPGN_CreateSocket(WIFI_SOCKET_IP_PROTOCOL_TCP, WIFI_SOCKET_IP_VERSION_4);

    if (status >= 0)
    {    
        /* Wifi connect.  */
        nx_wifi_socket[entry_index].socket_id = status;
...
UCHAR   entry_index;
    ...
    /* Close connection.  */
    wifi_err = R_WIFI_SX_ULPGN_CloseSocket(nx_wifi_socket[entry_index].socket_id);
    /* Reset the entry.  */
