...
static wwd_result_t wwd_wifi_join_wait_for_complete( host_semaphore_type_t *semaphore, wwd_interface_t interface )
{
    wwd_result_t result;
    ...
    while (!done)
    {
        result = host_rtos_get_semaphore( semaphore, DEFAULT_JOIN_ATTEMPT_TIMEOUT / 10, WICED_FALSE );
        ...
        done = ( ( host_rtos_get_time() - start_time ) >= DEFAULT_JOIN_ATTEMPT_TIMEOUT );
    }
...
wwd_result_t wwd_wifi_join( const wiced_ssid_t* ssid, wiced_security_t auth_type, const uint8_t* security_key, uint8_t key_length, host_semaphore_type_t* semaphore, wwd_interface_t interface )
{
    ...
    if ( result == WWD_SUCCESS )
    {
        ...
        {
            result = wwd_wifi_join_wait_for_complete( &join_sema, interface );
...
wwd_result_t wwd_wifi_join_specific( const wiced_scan_result_t* ap, const uint8_t* security_key, uint8_t key_length, host_semaphore_type_t* semaphore, wwd_interface_t interface )
{
    ...
    if ( result == WWD_SUCCESS )
    {
        ...
        if ( result == WWD_SUCCESS && semaphore == NULL )
        {
            result = wwd_wifi_join_wait_for_complete( &join_semaphore, interface );
        }
        else
        {
...
static wwd_result_t wwd_pno_add_ssid_network( wiced_ssid_t *ssid, wiced_security_t security )
{
    ...
    /* Look for infrastructure mode networks only, otherwise look for IBSS only. */
    /* XXX TODO: API should accept bss_type instead of hardcoding it. */
    pfn_net.infra = WICED_TRUE;
